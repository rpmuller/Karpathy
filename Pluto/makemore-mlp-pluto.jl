...
# ╔═╡ 533cd05c-3730-4309-880d-1c1d4f59f783
using Flux

# ╔═╡ 308ae6b8-c4b3-49bc-8224-a0ba297952b3
using Flux: softmax, crossentropy

# ╔═╡ 31a647de-6117-418e-b5d6-aaed0eb0b5a8
using Statistics

# Read names.txt into words array:

# ╔═╡ 41925ebf-b140-4313-a08e-0d981371047d
md"""
...
# ╔═╡ e2c4c0d7-e2a6-4a1a-86dc-84703c67637f
md"Read names.txt into words array"

# ╔═╡ 0899a9c0-41fa-4e97-92cf-0c4027f0bded
words = split(read("names.txt",String),"\n")

# ╔═╡ 4fd367d8-9838-41a1-84b6-54d227f1f431
length(words)

# ╔═╡ 834b8024-9a1b-42cd-ad69-dfb19ad1246c
chars = ".abcdefghijklmnopqrstuvwxyz"

# ╔═╡ 4a4b4b68-90fa-46a7-bac9-faee4e460f3e
stoi = Dict( s => i for (i,s) in enumerate(chars))

# ╔═╡ 4b5edd1d-7fa6-4c50-9f45-f1ee36f1955e
itos = Dict( i => s for (i,s) in enumerate(chars))

# ╔═╡ 060a42d6-4436-4117-a206-2311d16bdc1a
md"""
We're going to follow what Karpathy does pretty closely. The section of 
he FluxML docs called 
...
# ╔═╡ 063c3f22-3dba-41b3-9de2-0325b7e3ab8a
block_size = 3 # context length: how many chars to we use to predict next one?

# ╔═╡ 6d7faeb9-22dc-4180-a6fa-069ec5b0c29e
...
# ╔═╡ 25ef0c8c-4444-4be2-88cb-ec95969b52f8
md"""
Build datasets.
"""

# ╔═╡ d1ea0dd1-5c67-4c38-a208-fe44d9da761e
n1 = 8*length(words)÷10
#n1 = convert(Int64,floor(0.8*length(words)))

# ╔═╡ 6b063573-f8a2-495d-bd0b-4728485dbca5
n2 = 9*length(words)÷10

# ╔═╡ 5bda733a-4a3e-4134-a358-1888a9032003
Xtr,Ytr = build_dataset(words[1:n1])

# ╔═╡ 7c90cc3a-17d0-45d5-a60f-4b1fe7e95190
Xdev,Ydev = build_dataset(words[n1:n2])

# ╔═╡ ece4d7a9-efb3-4db0-a585-31ebe4bca3b1
Xte,Yte = build_dataset(words[n2:end])

# ╔═╡ b1f6877f-f9d2-4648-806a-2566fa13d4ec
X,Y = build_dataset(words)

# ╔═╡ 15004d15-f766-4da6-b1a7-3eb30b0222b0
md"We don't have a simple way of embedding a matrix like torch does, 
so we'll do it by hand for now to make it work, and worry about making it fast later."

# ╔═╡ 4982f505-83a8-4443-b9a4-1fe4641c5fb5
C = randn((27,2))  # Build embedding lookup table C.

# ╔═╡ b97359e1-355e-43de-8ffd-1248deb23e4f
C[X,:]

# ╔═╡ 2cb3aa5d-562e-4f5a-8302-6d0e31755d5c
C[X]

# ╔═╡ c7f75be9-f6bd-4fc6-9bfb-f41ef27876e2
W1 = randn(6,100)

# ╔═╡ f085f1b6-abd1-4c66-9bcb-973c24891dd4
b1 = randn(100)'

# ╔═╡ d4a5b614-5778-4e41-ac03-e49d91c6b603
W2 = randn(100,27)

# ╔═╡ 1897f565-450c-43b7-b5d0-befff2fed710
b2 = randn(27)';

# ╔═╡ 508be201-f701-4015-90f4-c24fe091b687
params = Flux.params(C,W1,b1,W2,b2)

# Forward pass

# ╔═╡ 95df36b4-0483-404d-bbf4-369ea7c07aa0
md"Forward Pass"

# ╔═╡ 0223f121-845e-421c-964f-f29d7552ef29
embed(X,C) = hcat(C[X[:,1],:],C[X[:,2],:],C[X[:,3],:])

# ╔═╡ e6e11024-9004-4048-8373-0a98c9cc06ac
embed(X,C)

# ╔═╡ fef93b38-8470-416a-ad58-3dc05712705b
function predict(X)
	# Build the embedded input matrix:
    Xemb = embed(X,C)
    h = tanh.(Xemb*W1 .+ b1)
    return h*W2 .+ b2
end

# ╔═╡ a017b69d-fb95-4569-bca6-8bb401634ec3
function mloss(X,Y)
    logits = predict(X)
...
# ╔═╡ a54036f8-9541-4f73-a327-ede2d8a7cf2a
logits = predict(X)

# ╔═╡ 8bb8766a-81b6-45e1-90c4-b3346702525a
softmax(logits)

# ╔═╡ 6c7d52d2-5a23-458d-b616-92148f4af5a3
function softmax0(logits)
	prob = exp.(logits)
	for i in 1:size(prob,2)
		d = sum(prob[i,:])
		prob[i,:] /= d
	end
	return prob
...
# ╔═╡ 3b7c78cb-00f2-4349-975e-2ab384426ebf
md"
My function is computing the transpose of the softmax. 
Need to think a little more carefully about indexes.
"

# ╔═╡ daff9c56-b9a3-4a5f-b258-a861e08e0bea
m = [0.1 0.1; 0.1 0.5]

# ╔═╡ 372e8afc-768f-458a-a669-2d77e1da8e43
softmax(m)

# ╔═╡ ddda90c7-81ee-431b-980f-bcfcde6ada11
softmax0(m)

# ╔═╡ d2410715-5c09-418b-ab41-459b2b79540a
softmax0(logits)

# ╔═╡ 315b8b02-eeb3-4080-93eb-80f43cb001ba
...
manifest_format = "2.0"
project_hash = "62be308e0fc80dd084572c4664df1a35a174dd37"
